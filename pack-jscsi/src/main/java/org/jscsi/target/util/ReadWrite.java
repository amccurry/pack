...
public final class ReadWrite {
  ...
  /**
   * Reads an unsigned 3-byte integer from three consecutive big-endian-ordered
   * bytes of a {@link ByteBuffer} object and returns the value.
   * 
   * @param buffer
   *          the {@link ByteBuffer} containing the bytes
   ...
   * <p>
   * If the String does not end with a null character, one will be appended. If
   * the String's length is larger than the specified <i>bufferSize</i>, all but
   * the last ByteBuffer will have <i>capacity() = bufferSize</i>, the last one
  ...
  public static ByteBuffer[] stringToTextDataSegments(String string, int bufferSize) {
    ...
    for (int i = 0; i < numberOfParts; ++i) {
      ...
      if (bytesRemaining > bufferSize) {
        sub = string.substring(i * bufferSize, (i + 1) * bufferSize);
      } else {
        sub = string.substring(i * bufferSize);
      }
      segments[i] = stringToByteBuffer(sub);
      bytesRemaining -= sub.length();
    }
    return segments;
  }

  /**
   * Puts the characters in the passed String into a ByteBuffer of equal length
   * and returns it.
   * 
   * @param string
   *          any String
  ...
  /**
   * Splits the passed <i>value</i> into bytes and returns them in an array, in
   * big-endian format.
   * 
   * @param value
   *          the long to split
  ...
  /**
   * Writes the two least-significant big-endian-ordered bytes of an integer the
   * a specified position in a {@link ByteBuffer}.
   * 
   * @param buffer
