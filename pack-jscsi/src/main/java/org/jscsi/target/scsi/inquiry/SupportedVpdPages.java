...
public final class SupportedVpdPages implements IResponseData {
  ...
  /**
   * The total length of all fields that are not part of the Supported VPD page
   * list.
   */
  ...
  public static final byte[] SUPPORTED_VPD_PAGES = new byte[] { (byte) 0x00, // SUPPORTED_VPD_PAGES,
      ...
      (byte) 0x80 // UNIT_SERIAL_NUMBER, mandatory
  };
  ...
  public void serialize(ByteBuffer byteBuffer, int index) {
    ...
    // *** byte 0 ***
    /*
     * Peripheral Qualifier (bits 7 - 5): 000b A peripheral device having the
     * specified peripheral device type is connected to this logical unit. If
     * the device server is unable to determine whether or not a peripheral
     * device is connected, it also shall use this peripheral qualifier. This
     * peripheral qualifier does not mean that the peripheral device connected
     * to the logical unit is ready for access. Peripheral Device Type (bits 4 -
  ...
  /**
   * Returns <code>true</code> for those and only for those VPD Page Codes which
   * are supported by the jSCSI Target.
   * 
   * @param vitalProductDataPageName
   *          VPD Page Name whose support is inquired
   * @return <code>true</code> for those and only for those VPD Page Codes which
   *         are supported by the jSCSI Target
   */
