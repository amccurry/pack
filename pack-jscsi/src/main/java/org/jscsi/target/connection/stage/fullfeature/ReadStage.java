...
public class ReadStage extends ReadOrWriteStage {
  ...
  public void execute(ProtocolDataUnit pdu)
      throws IOException, InterruptedException, InternetSCSIException, SettingsException {
    try (Scope s0 = TracerUtil.trace(getClass(), "execute")) {
      ...
      /*
       * If ImmediateData=Yes has been negotiated, then a phase collapse has to
       * take place, i.e. the status is sent in the last Data-In PDU. Otherwise
       * a separate SCSI Response PDU must follow.
       */
