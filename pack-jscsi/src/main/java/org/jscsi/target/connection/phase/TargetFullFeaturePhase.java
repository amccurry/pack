...
public final class TargetFullFeaturePhase extends TargetPhase {
  ...
  /**
   * While this variable is <code>true</code> the phase is still running, either
   * executing a specific stage or waiting for the next one to begin.
   */
  ...
  public boolean execute()
    ...
    while (running) {
      ...
      case SCSI_COMMAND:
        if (connection.getTargetSession()
                      .isNormalSession()) {
          final SCSICommandParser parser = (SCSICommandParser) bhs.getParser();
          ScsiOperationCode scsiOpCode = ScsiOperationCode.valueOf(parser.getCDB()
                                                                         .get(0));
...
//              LOGGER.info("byte 4 {}", Integer.toHexString(buffer.get(4)));
      ...
      case LOGOUT_REQUEST:
        ...
        break;
      default:
        LOGGER.error("Recieved unsupported opcode for " + pdu.getBasicHeaderSegment()
                                                             .getOpCode());
        stage = new UnsupportedOpCodeStage(this);
      }
