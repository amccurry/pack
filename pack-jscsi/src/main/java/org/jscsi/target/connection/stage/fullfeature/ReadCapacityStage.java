...
public final class ReadCapacityStage extends TargetFullFeatureStage {
  ...
  public void execute(ProtocolDataUnit pdu)
    ...
    final ScsiOperationCode opCode = ScsiOperationCode.valueOf(parser.getCDB()
                                                                     .get(0));
    ReadCapacityCdb cdb;
    ...
    /*
     * Everything is fine, carry on. The PMI bit of the command descriptor block is
     * ignored, since there is no way to know if
     * "substantial vendor specific delay in data transfer may be encountered" after
     * the address in the LOGICAL BLOCK ADDRESS field. Therefore we always try to
     * return the whole length of the storage medium.
     */
    ...
    if (storageModule.checkBounds(cdb.getLogicalBlockAddress(), 0) != 0) {
      // invalid, log error, send error PDU, and return
      LOGGER.error("encountered " + cdb.getClass() + " in ReadCapacityStage with " + "LOGICAL BLOCK ADDRESS = "
      ...
      if (cdb instanceof ReadCapacity10Cdb)
        parameterData = new ReadCapacity10ParameterData(storageModule.getSizeInBlocks(), // returnedLogicalBlockAddress
            storageModule.getBlockSize());// logicalBlockLengthInBytes
      else
        parameterData = new ReadCapacity16ParameterData(storageModule.getSizeInBlocks(), // returnedLogicalBlockAddress
            storageModule.getBlockSize());// logicalBlockLengthInBytes
