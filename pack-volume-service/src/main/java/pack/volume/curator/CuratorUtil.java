...
public class CuratorUtil {
  ...
  private static final String VOLUME_SERVERS_HOSTS = "/volume-servers/hosts";
  ...
  private static String getHostsPath(String zkPrefix) {
    if (zkPrefix == null || zkPrefix.isEmpty()) {
      return VOLUME_SERVERS_HOSTS;
    } else {
      return zkPrefix + VOLUME_SERVERS_HOSTS;
    }
  }
  ...
  public static List<PackVolumeHostEntry> getRegisteredServers(CuratorFramework curatorFramework, String zkPrefix)
      throws Exception {
    if (curatorFramework == null) {
    ...
    List<PackVolumeHostEntry> entries = new ArrayList<>();
    for (String s : list) {
      List<String> parts = PORT_SPLITTER.splitToList(s);
      entries.add(PackVolumeHostEntry.builder()
