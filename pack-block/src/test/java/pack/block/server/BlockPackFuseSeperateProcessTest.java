...
public class BlockPackFuseSeperateProcessTest {
  ...
  private static final String TEST_BLOCK_PACK_FUSE_SEPERATE_PROCESS = "testBlockPackFuseSeperateProcess";

  private static final Logger LOGGER = LoggerFactory.getLogger(BlockPackFuseSeperateProcessTest.class);
  ...
  private static File root = new File("./target/tmp/" + BlockPackFuseSeperateProcessTest.class.getName());
  ...
  public static void setup() throws IOException {
    ...
    Utils.exec(LOGGER, "sudo", "chown", "-R", user + ":" + user, root.getAbsolutePath());
    Utils.rmr(root);
  ...
  public void testBlockPackFuseSeperateProcess() throws Exception {
    Path volumePath = new Path("/BlockPackFuseTest/" + TEST_BLOCK_PACK_FUSE_SEPERATE_PROCESS);
    ...
    String hdfVolumePath = "/BlockPackFuseTest/" + TEST_BLOCK_PACK_FUSE_SEPERATE_PROCESS;
    String volumeName = TEST_BLOCK_PACK_FUSE_SEPERATE_PROCESS;
    String fuseMountLocation = mkdirs(new File(testRoot, "fuse")).getAbsolutePath();
    String fsMountLocation = mkdirs(new File(testRoot, "fs")).getAbsolutePath();
    String fsMetricsLocation = mkdirs(new File(testRoot, "metrics")).getAbsolutePath();
    String fsLocalCache = mkdirs(new File(testRoot, "cache")).getAbsolutePath();
    ...
    Process process = BlockPackFuseProcessBuilder.startProcess(false, fuseMountLocation, fsMountLocation,
        fsMetricsLocation, fsLocalCache, hdfVolumePath, zkConnection, zkTimeout, volumeName, logOutput, unixSock,
        libDir, numberOfMountSnapshots, volumeMissingPollingPeriod, volumeMissingCountBeforeAutoShutdown,
  ...
  private void waitForProcessToExit(Process process) throws InterruptedException, IOException {
