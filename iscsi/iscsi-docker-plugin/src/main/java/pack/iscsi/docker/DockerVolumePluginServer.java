...
import pack.iscsi.docker.json.Capabilities;
import pack.iscsi.docker.json.CapabilitiesResponse;
import pack.iscsi.docker.json.CreateRequest;
import pack.iscsi.docker.json.Err;
import pack.iscsi.docker.json.GetRequest;
import pack.iscsi.docker.json.GetResponse;
import pack.iscsi.docker.json.Implements;
import pack.iscsi.docker.json.ListResponse;
import pack.iscsi.docker.json.MountUnmountRequest;
import pack.iscsi.docker.json.PathRequest;
import pack.iscsi.docker.json.PathResponse;
import pack.iscsi.docker.json.RemoveRequest;
import pack.iscsi.docker.json.Volume;
...
public abstract class DockerVolumePluginServer {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DockerVolumePluginServer.class);
  ...
  public DockerVolumePluginServer(boolean global, String sockFile) {
    this.global = global;
    this.sockFile = sockFile;
  }

  protected abstract VolumeStorageControl getPackStorage() throws Exception;

  public void runServer() throws Exception {
    VolumeStorageControl packStorage = getPackStorage();
  ...
  private void debugInfo(Request request) {
    LOG.debug("pathInfo {} contextPath {}", request.pathInfo(), request.contextPath());
    Set<String> attributes = new TreeSet<>(request.attributes());
    for (String attribute : attributes) {
      LOG.debug("attribute {} {}", attribute, request.attribute(attribute));
    }
    Set<String> headers = new TreeSet<>(request.headers());
    for (String header : headers) {
      LOG.debug("header {} {}", header, request.headers(header));
    }
